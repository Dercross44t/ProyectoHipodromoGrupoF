...
namespace ProyectoHipodromoGrupoF.AccesoADatos
{
    public class VeterinarioRepository
    {
        ...
        public void EliminarVeterinario(string codigo, string usuarioActual)
        {
            ...
            using var comando = new NpgsqlCommand(
                "CALL public.eliminar_veterinario($1)", conexion);
        ...
        public string ObtenerCodigoVeterinarioPorUsuario(string usuarioActual)
        {
            using var conexion = _conexionDB.ObtenerConexion();
            using var comando = new NpgsqlCommand(
                "SELECT public.obtener_codigo_veterinario_por_usuario($1)", conexion);
        ...
        {
            var historiales = new List<HistorialVeterinario>();
            using var conexion = _conexionDB.ObtenerConexion();
            using var comando = new NpgsqlCommand(
            ...
            while (lector.Read())
            {
                historiales.Add(MapearHistorial(lector));
            }
        ...
        public void InsertarHistorial(HistorialVeterinario historial, string usuarioActual)
        {
            ...
            comando.Parameters.AddWithValue(historial.Diagnostico ?? string.Empty);
            comando.Parameters.AddWithValue(historial.Tratamiento ?? string.Empty);
